...
@section("page_breadcrumbs",Breadcrumbs::render("admin.admins.index", $title))
@section("content")
    <div class="row">
        <!-- ============================================================== -->
        <!-- basic table  -->
            ...
            <div class="card">
                <h5 class="card-header">
                    <div class="float-left">
                        @if(current_admin()->can("create-admins"))
                            <a href="{{route("admin.admins.create")}}" class="btn btn-primary"><i
                                    class="fas fa-plus fa-fw"></i> Create</a>
                        @endif
                        @if(current_admin()->can("delete-admins"))
                            <a id="bulkDeleteBtn" href="{{route("admin.admins.massDestroy")}}" class="btn btn-danger"><i
                            ...
                            @foreach($admins as $admin)
                                <tr data-entry-id="{{$admin['id']}}">
                                        ...
                                        @if(current_admin()->id != $admin['id'])
                                            <label class="custom-control custom-checkbox">
                                                <input type="checkbox"
                                                       ...
                                                       name="bulkSelect[]" value="{{$admin["id"]}}"><span
                                    ...
                                    <td style="width: 22%;">
                                        ...
                                        @if(current_admin()->id != $admin['id'] && current_admin()->can("update-admins"))
                                        ...
                                        @else
                                            <a title="Edit Profile"
                                               ...
                                               class="btn btn-success btn-sm"><i
                                                    class="fas fa-edit fa-fw"></i></a>
                                        @endif
                                        @if(current_admin()->id != $admin['id'] && current_admin()->can("update-profile"))
                                            <a title="Change Password"
                                               href="{{route("admin.admins.password.change", ["admin" => $admin['id']])}}"
                                        ...
                                        @else
                                            <a title="Edit Profile"
                                               ...
                                               class="btn btn-brand btn-sm"><i
                                                    class="fas fa-lock fa-fw"></i></a>
                                        @endif
                                        @if(current_admin()->id != $admin['id'] && current_admin()->can("read-admins"))
