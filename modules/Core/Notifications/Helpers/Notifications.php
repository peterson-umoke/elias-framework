...
use Modules\Core\Notifications\Exceptions\MissingPresetNotificationException;
use Modules\Core\Notifications\Helpers\Session as BasicSession;
...
class Notifications
{
     ...
     * @throws MissingPresetNotificationException
     */
    public function preset(string $presetName, array $overrideValues = []): self
    {
        $presetValues = config('notifications.preset-messages.'.$presetName);
    ...
    {
        return $this->session->get('notifications.message');
    }
    ...
    {
        return $this->session->get('notifications.type');
    }
}
