...
@section("page_breadcrumbs",Breadcrumbs::render("admin.posts.categories.categories.index", $title))
@section('content')
    <div class="row">
        <!-- ============================================================== -->
        <!-- basic table  -->
            ...
            <div class="card">
                <h5 class="card-header">
                    <div class="float-left">
                        @if(current_admin()->can("create-posts-categories"))
                            <a href="{{route("admin.posts.categories.create")}}" class="btn btn-primary"><i
                                    class="fas fa-plus fa-fw"></i> Create</a>
                        @endif
                        {{--                        @if(current_admin()->can("delete-posts-categories"))--}}
                        {{--                            <a id="bulkDeleteBtn" href="{{route("admin.posts.categories.massDestroy")}}" class="btn btn-danger"><i--}}
                        ...
                        <table id="dataThings" class="table table-striped table-bordered initDatatables">
                            ...
                            @foreach($all as $single)
                                <tr data-entry-id="{{$single->id}}">
                                    ...
                                    @endforeach
                                    <td style="width: 22%;">
                                        {!! Form::open(["route" => ["admin.posts.categories.destroy", $single->id],"method" => "delete"]) !!}
                                        @if(current_admin()->can("update-posts-categories"))
                                            <a title="Edit Post Category"
                                               href="{{route("admin.posts.categories.edit", $single->id)}}"
                                               class="btn btn-success btn-sm"><i
                                                    class="fas fa-edit fa-fw"></i></a>
                                        @endif
                                        @if(current_admin()->can("read-posts-categories"))
                                            <a title="Preview Post Category"
                                               href="{{route("admin.posts.categories.show",$single->id)}}"
                                               class="btn btn-primary btn-sm">
                                                <i class="fas fa-eye fa-fw"></i>
                                            </a>
                                        @endif
                                        @if(current_admin()->can("delete-posts-categories"))
                                            <button title="Delete Post Category" type="submit" class="btn btn-danger btn-sm"
