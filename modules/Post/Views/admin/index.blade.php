...
@section("page_breadcrumbs",Breadcrumbs::render("admin.posts.index", $title))
@section('content')
    <div class="row">
        <!-- ============================================================== -->
        <!-- basic table  -->
            ...
            <div class="card">
                <h5 class="card-header">
                    <div class="float-left">
                        @if(current_admin()->can("create-posts"))
                            <a href="{{route("admin.posts.create")}}" class="btn btn-primary"><i
                                    class="fas fa-plus fa-fw"></i> Create</a>
                        @endif
                        {{--                        @if(current_admin()->can("delete-posts"))--}}
                        {{--                            <a id="bulkDeleteBtn" href="{{route("admin.posts.massDestroy")}}" class="btn btn-danger"><i--}}
                        ...
                        <table id="dataThings" class="table table-striped table-bordered initDatatables">
                            ...
                            @foreach($posts as $post)
                                <tr data-entry-id="{{$post->id}}">
                                    ...
                                    @endforeach
                                    <td style="width: 22%;">
                                        {!! Form::open(["route" => ["admin.posts.destroy", $page->id],"method" => "delete"]) !!}
                                        @if(current_admin()->can("update-posts"))
                                            <a title="Edit Post"
                                               href="{{route("admin.posts.edit", $page->id)}}"
                                               class="btn btn-success btn-sm"><i
                                                    class="fas fa-edit fa-fw"></i></a>
                                        @endif
                                        @if(current_admin()->can("read-posts"))
                                            <a title="Preview Post"
                                               href="{{route("admin.posts.show",$page->id)}}"
                                        ...
                                        <a title="View Post"
                                           target="_blank"
                                           href="{{route("posts.show",$page->name)}}"
                                           class="btn btn-brand btn-sm">
                                            <i class="fas fa-globe fa-fw"></i>
                                        </a>
                                        @if(current_admin()->can("delete-posts"))
                                            <button title="Delete Post" type="submit" class="btn btn-danger btn-sm"
